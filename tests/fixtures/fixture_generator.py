...
def generate_pdf_with_bibtex(output_path: str, num_refs: int = 10) -> str:
    ...
    story.append(Paragraph(
        "This document includes BibTeX formatted references.",
        styles['Normal']
    ))
    story.append(Spacer(1, 0.3 * inch))
...
def generate_three_column_pdf(output_path: str, num_refs: int = 60) -> str:
    ...
    frame2 = Frame(
        doc.leftMargin + frame_width + 12, doc.bottomMargin,
        frame_width, doc.height,
        id='col2'
    )
    frame3 = Frame(
        doc.leftMargin + 2 * (frame_width + 12), doc.bottomMargin,
        frame_width, doc.height,
        id='col3'
    )
...
def generate_pdf_without_ref_header(output_path: str, num_refs: int = 25) -> str:
    ...
    story.append(Paragraph(
        "This concludes our study. See citations below.",
        styles['Normal']
    ))
    story.append(Spacer(1, 0.3 * inch))
