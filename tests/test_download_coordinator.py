...
from src.models import Reference, DownloadStatus, DownloadSource, DownloadResult
...
class TestDownloadCoordinator(unittest.TestCase):
    ...
    def test_fallback_chain_execution(self):
        ...
        with patch.object(self.coordinator.downloaders[0], 'can_download', return_value=True), \
             patch.object(self.coordinator.downloaders[0], 'download', return_value=mock_fail_result), \
             patch.object(self.coordinator.downloaders[1], 'can_download', return_value=True), \
             patch.object(self.coordinator.downloaders[1], 'download', return_value=mock_success_result):
            
            result = self.coordinator._try_downloaders(self.test_reference, self.temp_dir / "test.pdf")
    ...
    def test_successful_download(self):
        ...
        with patch.object(self.coordinator.downloaders[0], 'can_download', return_value=True), \
             patch.object(self.coordinator.downloaders[0], 'download', return_value=mock_result):
