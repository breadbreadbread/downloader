...
class TestExtractionFallbackManager(unittest.TestCase):
    ...
    def test_should_trigger_fallbacks_below_threshold(self):
        ...
        self.assertTrue(self.fallback_manager.should_trigger_fallbacks(self.sample_result))
    ...
    def test_should_not_trigger_fallbacks_above_threshold(self):
        ...
        self.assertFalse(self.fallback_manager.should_trigger_fallbacks(self.sample_result))
    ...
    def test_extract_from_tables_with_reference_table(self):
        ...
        mock_page.extract_tables.return_value = [
            [
                ["1", "Smith J., Johnson A.", "2023", "Machine Learning Advances", "J. AI Research", "15(3)", "123-145"],
                ["2", "Brown K., Davis L.", "2022", "Deep Learning Methods", "Neural Networks", "12(2)", "45-67"]
            ]
        ]
    ...
    def test_looks_like_reference_negative(self):
        ...
        non_reference_text = "This is just a regular sentence about something completely different."
        
        self.assertFalse(self.fallback_manager._looks_like_reference(non_reference_text))
    ...
    def test_deduplicate_references(self):
        ...
        existing_fingerprints = self.fallback_manager._create_reference_fingerprint_set(existing_refs)
        
        new_refs = [
            Reference(raw_text="ref3", doi="10.5678/new.2023"),  # New
            Reference(raw_text="ref4", doi="10.1234/example.2023"),  # Duplicate DOI
            Reference(raw_text="ref5", title="Paper Title", year=2023),  # Duplicate title+year
            Reference(raw_text="ref6", title="Different Title", year=2023)  # New
        ]
        
        unique_refs = self.fallback_manager._deduplicate_references(new_refs, existing_fingerprints)
    ...
    def test_apply_fallbacks_pdf_source(self):
        ...
        with patch.object(self.fallback_manager, '_extract_from_tables', return_value=[]), \
            ...
            self.assertTrue(any("Table fallback:" in error for error in enhanced_result.extraction_errors))
            self.assertTrue(any("BibTeX fallback:" in error for error in enhanced_result.extraction_errors))
    ...
    def test_apply_fallbacks_web_source(self):
        ...
        with patch.object(self.fallback_manager, '_extract_from_bibtex', return_value=[]), \
             patch.object(self.fallback_manager, '_extract_from_html_structure', return_value=[]):
            ...
            self.assertGreater(len(enhanced_result.extraction_errors), 0)
            self.assertTrue(any("BibTeX fallback:" in error for error in enhanced_result.extraction_errors))
            self.assertTrue(any("HTML structure fallback:" in error for error in enhanced_result.extraction_errors))
    ...
    def test_apply_fallbacks_no_trigger(self):
        ...
        result = ExtractionResult(source="test.pdf")
        result.references = [
            Reference(raw_text=f"Reference {i}") for i in range(10)
        ]
        
        mock_pdf = Mock()
        
        with patch.object(self.fallback_manager, '_extract_from_tables') as mock_tables, \
             patch.object(self.fallback_manager, '_extract_from_bibtex') as mock_bibtex:
    ...
    def test_apply_fallbacks_successful_extraction(self):
        ...
        fallback_refs = [
            ...
            Reference(raw_text="Fallback reference 2", title="Fallback Paper", year=2023)
        ]
        
        mock_pdf = Mock()
        
        with patch.object(self.fallback_manager, '_extract_from_tables', return_value=[]), \
             patch.object(self.fallback_manager, '_extract_from_bibtex', return_value=fallback_refs):
            ...
            self.assertEqual(len(enhanced_result.references), 3)  # 1 original + 2 fallback
            self.assertEqual(enhanced_result.total_references, 3)
            
            # Should not have error messages about failed fallbacks
            bibtex_errors = [e for e in enhanced_result.extraction_errors if "BibTeX fallback:" in e]
...
class TestTableExtractionHelpers(unittest.TestCase):
    ...
    def test_normalize_table_cells(self):
        ...
        table = [
            ["1", "Smith, J.", "2023", "Paper Title", "Journal Name", "15(3)", "123-145"],
            ["2", "Johnson, A.", "2022", "Another Paper", "Another Journal", "12(2)", "45-67"],
            ["", "", "", "", "", "", ""],  # Empty row
        ...
        self.assertIn("1 Smith, J. 2023 Paper Title Journal Name 15(3) 123-145", lines[0])
        self.assertIn("2 Johnson, A. 2022 Another Paper Another Journal 12(2) 45-67", lines[1])
