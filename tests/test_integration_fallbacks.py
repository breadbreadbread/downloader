...
from tests.test_fixtures import create_sample_html_with_lists, create_sample_html_with_citations, create_sample_text_with_bibtex
...
class TestPDFExtractorFallbacks(unittest.TestCase):
    ...
    def test_pdf_extraction_with_fallbacks_below_threshold(self, mock_pdfplumber_open, mock_exists):
        ...
        with patch.object(self.pdf_extractor.fallback_manager, 'apply_fallbacks') as mock_fallbacks:
    ...
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pdfplumber.open')
    def test_pdf_extraction_no_fallbacks_above_threshold(self, mock_pdfplumber_open, mock_exists):
        ...
        many_refs = "\n".join([f"{i}. Author {i} (202{i%10}). Paper {i}. Journal Name, {i}({i}), {i}-{i+10}." for i in range(10)])
        ...
        with patch.object(self.pdf_extractor.fallback_manager, 'should_trigger_fallbacks', return_value=False):
            with patch.object(self.pdf_extractor.fallback_manager, 'apply_fallbacks') as mock_fallbacks:
                result = self.pdf_extractor.extract("test.pdf")
    ...
    def test_pdf_extraction_table_fallback(self, mock_pdfplumber_open, mock_exists):
        ...
        mock_page.extract_tables.return_value = [
            [
                ["2", "Johnson A.", "2022", "Second paper", "Journal Name", "10(2)", "45-67"],
                ["3", "Brown K.", "2021", "Third paper", "Another Journal", "8(1)", "123-145"]
            ]
        ]
...
class TestWebExtractorFallbacks(unittest.TestCase):
    ...
    def test_web_extraction_with_fallbacks_below_threshold(self, mock_get):
        ...
        with patch.object(self.web_extractor.fallback_manager, 'apply_fallbacks') as mock_fallbacks:
            mock_fallbacks.return_value = ExtractionResult(
    ...
    def test_web_extraction_bibtex_fallback(self, mock_get):
        ...
        self.assertTrue(any("Machine Learning Techniques" in title for title in ref_titles))
...
class TestFallbackDeduplication(unittest.TestCase):
    ...
    def test_fallback_deduplication_by_doi(self, mock_pdfplumber_open, mock_exists):
        ...
        doi_refs = [ref for ref in result.references if ref.doi == "10.1234/example.2023"]
    ...
    def test_fallback_deduplication_by_title_year(self, mock_get):
        ...
        title_year_refs = [
            ...
            if ref.title and ref.year and "Machine Learning Advances" in ref.title and ref.year == 2023
        ]
