...
class DependencyValidator:
    ...
    def __init__(self, project_root: Path):
        ...
        self.results = {
            ...
            "pip_audit": {"status": "unknown", "output": "", "error": "", "vulnerabilities": []},
            "coverage": {"status": "unknown", "output": "", "error": "", "percentage": None}
        }
    ...
    def check_pip_check(self) -> bool:
        ...
        returncode, stdout, stderr = self.run_command([sys.executable, "-m", "pip", "check"])
    ...
    def check_pip_audit(self) -> bool:
        ...
        try:
            ...
            if unapproved_vulns:
                print(f"❌ pip audit found {len(unapproved_vulns)} unapproved vulnerabilities:")
                for vuln in unapproved_vulns:
                    print(f"  - {vuln.get('id', 'Unknown')}: {vuln.get('description', 'No summary')[:100]}...")
                self.results["pip_audit"]["status"] = "failed"
                return False
            else:
                print(f"⚠️  pip audit found {len(vulnerabilities)} known but approved vulnerabilities")
...
def main():
    ...
    parser.add_argument(
        "--coverage", 
        action="store_true", 
        help="Include test coverage validation"
    )
    parser.add_argument(
        "--verbose", 
        action="store_true", 
