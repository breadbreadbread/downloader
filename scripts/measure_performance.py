...
def measure_download_performance():
    ...
    print(
        f"  Coordinator overhead: {results['download_coordinator']['time_seconds']:.3f}s"
    )
...
def save_performance_baseline(extraction_results, download_results):
    ...
    for scenario, results in extraction_results.items():
        if scenario in baseline["targets"]:
            ...
            print(
                f"    Time: {results['time_seconds']:.2f}s (target: ≤{target['max_time']}s)"
            )
            print(
                f"    Memory: {results['memory_mb']:.1f}MB (target: ≤{target['max_memory']}MB)"
            )
