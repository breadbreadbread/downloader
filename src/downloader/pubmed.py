...
class PubMedDownloader(BaseDownloader):
    ...
    def download(
        ...
        try:
            ...
            if reference.doi:
                result = self._search_and_download(reference, reference.doi, output_path)
                if result and result.status == DownloadStatus.SUCCESS:
                    return result
            
            # Try searching by title
            if reference.title:
                result = self._search_and_download(reference, reference.title, output_path)
                if result and result.status == DownloadStatus.SUCCESS:
                    return result
