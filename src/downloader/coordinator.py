    ...
    def _try_downloaders(
        ...
        for downloader in self.downloaders:
            try:
                ...
                logger.info(
                    f"Attempting download with {downloader.__class__.__name__}"
                )
                ...
                if result:
                    if result.status == DownloadStatus.SUCCESS:
                        logger.info(
                            f"Successfully downloaded: {output_path}"
                        )
                        return result
                        ...
                        logger.debug(
                            f"Not found: {result.error_message}"
                        )
                        continue
            
            except Exception as e:
                logger.error(
                    f"Error with {downloader.__class__.__name__}: {str(e)}"
                )
                continue
        
        # All downloaders failed
        logger.warning(
            f"Failed to download paper for reference: {reference.title}"
        )
