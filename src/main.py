...
from src.models import DownloadResult, DownloadSource, DownloadStatus, DownloadSummary
...
Examples:
  ...
  python -m src.main --url https://example.com/paper --output ./downloads
    ...
    parser.add_argument(
        "--pdf", type=str, help="Path to PDF file to extract references from"
    )

    parser.add_argument(
        "--url", type=str, help="URL of web page to extract references from"
    )
