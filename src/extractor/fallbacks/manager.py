...
class ExtractionFallbackManager:
    ...
    def apply_fallbacks(
        ...
        if source_type == "pdf" and self.enable_table_fallback and pdf_object:
            ...
            if table_refs:
                ...
                if new_refs:
                    ...
                    existing_refs_set.update(
                        self._create_reference_fingerprint_set(new_refs)
                    )
                    logger.info(
                        f"Table fallback extracted {len(new_refs)} additional references"
                    )
                else:
                    result.extraction_errors.append(
                        "Table fallback: No new unique references found"
                    )
            else:
                result.extraction_errors.append(
                    "Table fallback: No reference tables detected"
                )
        ...
        if self.enable_bibtex_fallback:
            ...
            if bibtex_refs:
                ...
                if new_refs:
                    ...
                    logger.info(
                        f"BibTeX fallback extracted {len(new_refs)} additional references"
                    )
                else:
                    result.extraction_errors.append(
                        "BibTeX fallback: No new unique references found"
                    )
            else:
                result.extraction_errors.append(
                    "BibTeX fallback: No BibTeX blocks detected"
                )

        # Apply HTML structure fallback (web only)
        if (
            source_type == "web"
            and self.enable_html_structure_fallback
            and html_content
        ):
            ...
            if html_refs:
                ...
                if new_refs:
                    ...
                    logger.info(
                        f"HTML structure fallback extracted {len(new_refs)} additional references"
                    )
                else:
                    result.extraction_errors.append(
                        "HTML structure fallback: No new unique references found"
                    )
            else:
                result.extraction_errors.append(
                    "HTML structure fallback: No structured citation elements found"
                )
        ...
        if fallback_results:
            ...
            logger.info(
                f"Fallback strategies added {len(fallback_results)} references (total: {result.total_references})"
            )
        else:
            result.extraction_errors.append(
                "All fallback strategies failed to extract new references"
            )
        ...
        for line in lines:
            ...
            if len(line) > 20:  # Minimum length for a meaningful reference
                try:
                    ...
                    logger.debug(
                        f"Failed to parse table reference: {line[:50]}... - {str(e)}"
                    )
            ...
            if "author" in fields:
                ...
                if authors:
                    ...
                    if "," in first_author:
                        ...
                        ref.first_author_last_name = (
                            parts[-1] if parts else first_author
                        )
                ...
                try:
                    ...
                    for element in elements:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Failed to parse HTML reference: {text[:50]}... - {str(e)}"
                            )
    ...
    def _create_reference_fingerprint_set(self, references: List[Reference]) -> set:
        ...
        for ref in references:
            ...
            if ref.doi:
                ...
                raw_fingerprint = re.sub(
                    r"\s+", " ", ref.raw_text.lower().strip()[:100]
                )
    ...
    def _deduplicate_references(
        self, new_references: List[Reference], existing_fingerprints: set
    ) -> List[Reference]:
