...
from src.models import DownloadStatus, DownloadSummary
...
class ReportGenerator:
    ...
    def generate_reports(
        self, summary: DownloadSummary, report_name: str = "download_report"
    ) -> None:
        """
    ...
    def _format_text_report(self, summary: DownloadSummary) -> str:
        ...
        successful_results = [
            r for r in summary.results if r.status == DownloadStatus.SUCCESS
        ]
        ...
        # Failed downloads
        failed_results = [
            r for r in summary.results if r.status == DownloadStatus.FAILED
        ]
        if failed_results:
        ...
        # Skipped
        skipped_results = [
            r for r in summary.results if r.status == DownloadStatus.SKIPPED
        ]
        if skipped_results:
